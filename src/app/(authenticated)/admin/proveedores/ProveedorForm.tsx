...
const initial: ProveedorResult | null = null;
...
export default function ProveedorForm({
  ...
  const action = mode === "crear" ? crearProveedor : actualizarProveedor;
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <Field label="Nombre comercial" required full>
          <input
            ...
            placeholder="Suplementos del Norte"
            className="input"
          />
        </Field>
        ...
        <Field label="Teléfono">
          <input
            name="contacto_tel"
            type="tel"
            defaultValue={proveedor?.contacto_tel ?? ""}
            className="input"
          />
        </Field>
        ...
        >
          <input
            ...
            min={0}
            step={1}
            defaultValue={proveedor?.dias_credito ?? 0}
            className="input"
          />
        </Field>
      ...
      <div className="flex justify-end gap-2">
        <SubmitButton
          label={mode === "crear" ? "Crear proveedor" : "Guardar cambios"}
        />
      </div>
