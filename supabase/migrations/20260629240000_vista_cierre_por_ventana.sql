-- ============================================================================
-- 79 · vista_reporte_cierre por VENTANA de periodo (no por mes calendario)
--
-- Los flujos del cierre (enviado, devueltos, mermas, ventas, ajuste de almacén)
-- ahora se cuentan dentro de la ventana [fecha_inicio_cierre, fecha_cierre]
-- (o hasta now() si el cierre sigue abierto), en vez de por mes calendario.
-- Así respeta el encadenado y, en junio, cuenta ventas solo a partir del
...
with movs as (
  ...
  left join public.movimientos_inventario mi
    on mi.fecha >= c.fecha_inicio_cierre
   and mi.fecha < coalesce(c.fecha_cierre, now())
  group by c.id
),
pes as (
  select pm.cierre_id,
