-- ============================================================================
-- 75 · Devolución automática de vasos no usados en el llenado
--
-- Bug: op_registrar_llenado generaba devolución de CARTUCHOS no usados, pero
-- para VASOS solo manejaba el caso vasos_cargados > 0 (sumarlos a la máquina).
-- Los vasos planeados que el operador NO cargaba (devolución) NO generaban
-- ninguna devolucion_almacen, así que los vasos volvían físicamente al almacén
-- pero el sistema no los registraba (inventario subvaluado).
--
-- Fix: cuando vasos_planeados - vasos_cargados > 0, se crea una
-- devoluciones_almacen de vasos (pendiente_devolucion) ligada al surtido_item
-- del vaso, máquina y asignación. recibir_devolucion ya sabe procesar vasos
-- (reintegra a lotes.unidades_disponibles).
-- ============================================================================
...
begin
  ...
  -- ===== Vasos: carga a la máquina + devolución de no usados =====
  ...
  if v_vasos_planeados > 0 then
    ...
    -- surtido_item del vaso (para referencia/costo/lote en la devolución)
    ...
    -- Devolución de vasos no usados → al almacén (pendiente de recepción)
  ...
  -- Cierra check-in con datos de checkout
